...
if(isset($_POST['submit'])){
   ...
   $total_products = $_POST['quantity'];
   $total_price = $_POST['bill_amount'];

   $check_cart = $conn->prepare("SELECT * FROM `item` WHERE user_id = ?");
   ...
   if($check_cart->rowCount() > 0){
         ...
         $insert_order = $conn->prepare("INSERT INTO `orders`(cus_id, name, number, email, method, , quantity, bill_amount) VALUES(?,?,?,?,?,?,?)");
         $insert_order->execute([$user_id, $name, $number, $email, $method, $total_products, $total_price]);

         $delete_cart = $conn->prepare("DELETE FROM `item` WHERE cus_id = ?");
